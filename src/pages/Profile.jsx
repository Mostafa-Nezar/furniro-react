...
const Profile = () => {
  ...
  const handleImageChange = async e => {
    ...
    try {
      const res = await fetch(`https://furniro-back.vercel.app/api/upload/${user?.id}/update-image`, {
  ...
  const updatePhone = async (userId, newPhone) => {
    const res = await fetch(`https://furniro-back.vercel.app/api/auth/users/${userId}/phone`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ phoneNumber: newPhone }) });
    const data = await res.json();
    if (res.ok) {
    ...
    try {
      ...
      navigator.geolocation.getCurrentPosition(
        async (pos) => {
          ...
          const token = localStorage.getItem("token");
          const res = await fetch(
            `https://furniro-back.vercel.app/api/auth/users/${user.id}/location`,
            {
              method: "PATCH",
              headers: {
...
const Profile = () => {
  ...
  const handleImageChange = async (e) => {
    ...
    try {
      const res = await fetch(
        `https://furniro-back.vercel.app/api/upload/${user?.id}/update-image`,
        {
          method: "PATCH",
      ...
      togglePopup("Error uploading image");
    } finally {
  ...
  const renderNotificationsContent = () => (
    ...
    >
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h5 className="my-text-black mb-0">Notifications</h5>
        <button
          className="my-btn my-bg-primary text-white btn-md"
          onClick={() => {
            fetchNotifications();
            markAllAsReadInDB();
          }}
      ...
      {notifications.length > 0
        ? notifications.map((notif, i) => (
            <motion.div
              key={notif._id}
              className="d-flex align-items-center mb-3 rounded"
              style={{
                backgroundColor: "#fff",
  ...
  const updatePhone = async (userId, newPhone) => {
    const res = await fetch(
      `https://furniro-back.vercel.app/api/auth/users/${userId}/phone`,
      {
        method: "PATCH",
  ...
  const renderEmptyContent = (icon, title, subtitle) => (
    ...
    >
      <svg
        ...
        height="60"
        fill="#6c757d"
        className={`bi bi-${icon} mb-3`}
        viewBox="0 0 16 16"
      >
        <path
          d={
            icon === "favorite-border"
              ? "m8 2.748-.717-.737C5.6.281 2.514.878 1.4 3.053c-.523 1.023-.641 2.5.314 4.385C2.885 9.279 5.481 11.9 8 14.058c2.519-2.158 5.115-4.78 6.286-6.62.955-1.886.838-3.362.314-4.385C13.486.878 10.4.28 8.717 2.01L8 2.748z"
              : icon === "history"
              ? "M8 0a8 8 0 1 1 0 16A8 8 0 0 1 8 0zm0 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1zm-.5 2.5v5.5l4.5 2.7.7-1.2-3.8-2.3V3.5H7.5z"
              : icon === "location-on"
              ? "M8 16s6-5.686 6-10A6 6 0 0 0 2 6c0 4.314 6 10 6 10zm0-7a3 3 0 1 1 0-6 3 3 0 0 1 0 6z"
              : icon === "payment"
              ? "M2.5 4A1.5 1.5 0 0 0 1 5.5V6h14v-.5A1.5 1.5 0 0 0 13.5 4h-11zM1 7.5v5A1.5 1.5 0 0 0 2.5 14h11a1.5 1.5 0 0 0 1.5-1.5v-5H1zm1 2h12v1H2v-1z"
              : icon === "notifications"
              ? "M8 16a2 2 0 0 0 1.985-1.75c.017-.137-.097-.25-.235-.25h-3.5c-.138 0-.252.113-.235.25A2 2 0 0 0 8 16zM3 5a5 5 0 0 1 10 0v6a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V5z"
              : "M8 1.5a6.5 6.5 0 1 0 0 13 6.5 6.5 0 0 0 0-13zM8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0-2a5 5 0 1 0 0-10 5 5 0 0 0 0 10z"
          }
        />
      </svg>
    ...
    >
      {favoriteProducts.length > 0
        ? favoriteProducts.map((item, i) => (
            <motion.div
              key={i}
              className="d-flex mb-3 p-3 rounded bg-light"
              initial={{ x: -50, opacity: 0 }}
              animate={{ x: 0, opacity: 1 }}
              transition={{ delay: i * 0.1, duration: 0.4 }}
            >
              ...
              <button
                className="my-btn btn-outline-danger btn-sm"
              ...
              >
                <svg
                  ...
                  width="18"
                  height="18"
                  fill={favorites.includes(item.id) ? "#dc3545" : "#6c757d"}
      ...
      {cart.length > 0
        ? cart.map((item, i) => (
            ...
            >
              ...
              <button
                className="my-text-red border-0 bg-transparent my-btn btn-sm"
  ...
  const renderPhoneContent = () => {
    const handleSavePhone = async () => {
      if (!/^0\d{9,11}$/.test(phone)) {
        return togglePopup(
          "Invalid phone number. Must start with 0 and be 10–12 digits."
        );
      }
      try {
        await updatePhone(user.id, phone);
    ...
    return (
      <div className="p-3 rounded">
        ...
        />
        <button
          className="my-btn my-bg-green text-white"
      ...
      >
        {orders.length > 0
          ? orders.map((order, i) => (
              <motion.div
                key={order._id}
                className="mb-3 p-3 rounded my-bg-semi-white"
                initial={{ x: -50, opacity: 0 }}
                animate={{ x: 0, opacity: 1 }}
                transition={{ delay: i * 0.1, duration: 0.4 }}
              ...
              >
                <div className="d-flex align-items-center gap-2">
                  ...
                  <strong className="my-text-black">
                  ...
                  {["paid", "deliverd"].includes(order.status) ? (
                    ...
                    >
                      ...
                      >
                        ...
                        <path fill="currentColor" d="M25 8h10v9H25z" />
                      </g>
                    </svg>
                  )}
                  <span className="my-text-black">Status: {order.status}</span>
  ...
  const menuItems = [
    ...
    },
    {
      ...
      content: renderFavoritesContent(),
    },
    {
      id: "orders",
      icon: "box-seam",
    ...
    },
    {
      ...
      content: renderGenericContent("Payment Methods", "payment"),
    },
    {
      id: "notifications",
      icon: "bell",
    ...
    },
    {
      ...
      icon: "phone",
      title: "Phone",
      subtitle: user?.phoneNumber ? "phone saved" : "set phone number",
  ...
  if (!isAuthenticated)
    return (
      ...
      >
        ...
        <p className="text-muted">Please sign in to access your profile</p>
        <button
          className="my-btn my-bg-primary me-2"
          onClick={() => navigate("/signin")}
        >
          Sign In
        </button>
        <button
          className="my-btn my-bg-primary"
  ...
  return (
    <div className={`${theme ? "" : "bg-dark"}`}>
      ...
      <div className="container my-5">
          ...
          <div className="col-auto">
            ...
            >
              ...
              <label
                ...
                className="my-btn btn-sm my-bg-gray my-text-black position-absolute bottom-0 end-0"
              >
                {isUploading ? "..." : "✎"}
              </label>
              <input
        ...
        <motion.div
          className={`d-flex align-items-center justify-content-between p-3 px-4 rounded-3 shadow-sm my-5 ${
            theme ? "my-bg-black my-text-white" : "my-bg-white my-text-black"
          }`}
          transition={{ duration: 0.3 }}
          style={{ gap: "15px", border: "1px solid" }}
        >
          ...
          <motion.button
            ...
            className={`btn fw-bold ${
              theme ? "bg-white my-text-black" : "bg-dark text-white"
            }`}
            transition={{ duration: 0.3 }}
            style={{
              padding: "10px 15px",
        ...
        <div className="row g-3">
          {menuItems.map((item, i) => (
            <div className="col-6 col-md-4 col-lg-3" key={i}>
              ...
              >
                <motion.button
                  className={`w-100 d-flex flex-column align-items-center justify-content-center text-start ${
                    theme ? "my-bg-semiWhite" : "my-bg-darkGray"
                  }`}
                  style={{
                    borderRadius: 14,
                    padding: 16,
                    ...
                    border:
                      activeSection === item.id
                        ? "2px solid var(--primary, #b88e2f)"
                        : "1px solid rgba(0,0,0,0.08)",
                    boxShadow:
                      activeSection === item.id
                        ? "0 0 0 4px rgba(184,142,47,0.12)"
                        : "none",
                  }}
                  onClick={() =>
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      height: 44,
                      borderRadius: 12,
                      background: theme ? "#eef2f6" : "#2b2f36",
                    }}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      ...
                      <path
                        d={
                          item.icon === "heart"
                            ? "m8 2.748-.717-.737C5.6.281 2.514.878 1.4 3.053c-.523 1.023-.641 2.5.314 4.385C2.885 9.279 5.481 11.9 8 14.058c2.519-2.158 5.115-4.78 6.286-6.62.955-1.886.838-3.362.314-4.385C13.486.878 10.4.28 8.717 2.01L8 2.748z"
                            : item.icon === "box-seam"
                            ? "M8.44.146a.5.5 0 0 0-.88 0L5.162 3H2.5a.5.5 0 0 0-.5.5v9.6a.5.5 0 0 0 .257.437l6 3.2a.5.5 0 0 0 .486 0l6-3.2a.5.5 0 0 0 .257-.437V3.5a.5.5 0 0 0-.5-.5h-2.662L8.44.146zM3 4h2.661l1.377 2.297a.5.5 0 0 0 .866 0L9.339 4H13v1.5l-5 2.667L3 5.5V4z"
                            : item.icon === "geo-alt"
                            ? "M8 16s6-5.686 6-10A6 6 0 0 0 2 6c0 4.314 6 10 6 10zm0-7a3 3 0 1 1 0-6 3 3 0 0 1 0 6z"
                            ...
                            ? "M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16zm.5-5a.5.5 0 0 1-1 0V6a.5.5 0 0 1 1 0v5zm0 2.5a.5.5 0 0 1-.5.5h-.5a.5.5 0 0 1 0-1h.5a.5.5 0 0 1 .5.5z"
                            : item.icon === "phone"
                            ? "M21 16.42v3.536a1 1 0 0 1-.93.998Q19.415 21 19 21C10.163 21 3 13.837 3 5q0-.414.046-1.07A1 1 0 0 1 4.044 3H7.58a.5.5 0 0 1 .498.45q.034.344.064.552A13.9 13.9 0 0 0 9.35 8.003c.095.2.033.439-.147.567l-2.158 1.542a13.05 13.05 0 0 0 6.844 6.844l1.54-2.154a.46.46 0 0 1 .573-.149a13.9 13.9 0 0 0 4 1.205q.208.03.55.064a.5.5 0 0 1 .449.498"
                            : "M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16zM8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4z"
                        }
                      />
                    </svg>
                    {item.icon === "bell" && unreadCount > 0 && (
                      <span
                        className="position-absolute top-0 start-100 translate-middle badge my-bg-red d-flex justify-content-center"
                  ...
                  <div className="text-center">
                    <span
                      className="d-block my-text-black"
        ...
        <AnimatePresence>
          {activeSection && (
            ...
            >
              ...
              >
                {menuItems.find((m) => m.id === activeSection)?.content || null}
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        <motion.button
          className="my-btn my-bg-red my-text-semiWhite mt-3"
...
//         <div className="row g-3">
//           {menuItems.map((item, i) => (
//             <div className="col-6 col-md-6 col-lg-6" key={i}>
//               <motion.div
//                 initial={{ y: 20, opacity: 0 }}
//                 animate={{ y: 0, opacity: 1 }}
//                 transition={{ delay: i * 0.05, duration: 0.3 }}
