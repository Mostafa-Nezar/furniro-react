import Features from "../comps/Features.jsx";
import Landing from "../comps/Landing.jsx";

const Contact = () => {


  return (
    <>
      <Landing land="Contact" />
    <div className="vaidation p-5 m-5">
      <h4 style={{ fontWeight: 500 }} className="p-1 text-center">
        Get In Touch With Us
      </h4>
      <p className="text-center text-black-50">
        For More Information About Our Products &amp; Services Please Feel Free To
        Drop Us <br />
        An Email Our Staff Always Be There To Help You Cut Do Not Hestate !
      </p>

      <div className="row">
        <div className="col-md-6 p-5 pe-md-0 fieldsx mb-5">
          <div>
            <div className="mb-5">
              <svg
                style={{ position: "relative", top: "-8px", left: "-8px" }}
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="24"
                viewBox="0 0 32 32"
              >
                <path
                  fill="currentColor"
                  d="M16 2A11.013 11.013 0 0 0 5 13a10.9 10.9 0 0 0 2.216 6.6s.3.395.349.452L16 30l8.439-9.953c.044-.053.345-.447.345-.447l.001-.003A10.9 10.9 0 0 0 27 13A11.013 11.013 0 0 0 16 2m0 15a4 4 0 1 1 4-4a4.005 4.005 0 0 1-4 4"
                />
                <circle cx="16" cy="13" r="4" fill="none" />
              </svg>
              <span>
                <span>Address</span>
                <p className="ml-4">
                  236 5 th SE Avenue, <br /> New York NY10000, <br /> United
                  States
                </p>
              </span>
            </div>

            <div className="my-5">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="24"
                viewBox="0 0 24 24"
              >
                <path
                  fill="currentColor"
                  d="m20.487 17.14l-4.065-3.696a1.001 1.001 0 0 0-1.391.043l-2.393 2.461c-.576-.11-1.734-.471-2.926-1.66c-1.192-1.193-1.553-2.354-1.66-2.926l2.459-2.394a1 1 0 0 0 .043-1.391L6.859 3.513a1 1 0 0 0-1.391-.087l-2.17 1.861a1 1 0 0 0-.29.649c-.015.25-.301 6.172 4.291 10.766C11.305 20.707 16.323 21 17.705 21c.202 0 .326-.006.359-.008a.992.992 0 0 0 .648-.291l1.86-2.171a.997.997 0 0 0-.085-1.39"
                />
              </svg>
              <span>
                <span>Phone</span>
                <p className="ml-4">
                  Moblie: +(84) 546-67895 <br /> Hotline: +(84) 456-67895
                </p>
              </span>
            </div>

            <div>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="24"
                viewBox="0 0 16 16"
              >
                <path
                  fill="currentColor"
                  d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0M8 3.5a.5.5 0 0 0-1 0V9a.5.5 0 0 0 .252.434l3.5 2a.5.5 0 0 0 .496-.868L8 8.71z"
                />
              </svg>
              <span>
                <span>Working Time</span>
                <p className="ml-4">
                  Monday-Friday:9:00 - <br /> 22:00 <br /> Saturday-Sunday: 9:00
                  - <br /> 21:00
                </p>
              </span>
            </div>
          </div>
        </div>

        <div className="col-md-6 p-5 ps-md-0 fields">
          <div>
            <label htmlFor="name">Your name</label>
            <br />
            <input
              type="text"
              className="namefield"
              name="name"
              id="name"
              placeholder="Abc"
            />
          </div>

          <div>
            <label htmlFor="mail">email address</label>
            <br />
            <input
              type="email"
              className="mailfield"
              name="mail"
              id="mail"
              placeholder="example@mail."
            />
          </div>

          <div>
            <label htmlFor="message">message</label>
            <br />
            <input
              type="text"
              className="messagefiled"
              name="message"
              id="message"
              placeholder="Hi ! i,d like to ask about"
            />
          </div>

          <div>
            <label htmlFor="subject">subject</label>
            <br />
            <input
              type="text"
              className="subjectfiled"
              name="subject"
              id="subject"
              placeholder="Hi ! i,d like to ask about"
            />
          </div>

          <button type="button" className="submit">
            submit
          </button>
        </div>
      </div>
    </div>
      <Features />
    </>
  );
};

export default Contact;
